...
double min_abs_diag(const std::pair<int, int> sz, const std::vector<std::vector<double>> &A) {
  auto d = diag(sz, A);
  if (d.empty()) {
    return 0.0;
  }
...
std::vector<double> diag(const std::pair<int, int> sz, const std::vector<std::vector<double>> &A) {
