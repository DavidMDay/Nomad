...
std::pair<int, int> test_sip(const std::vector<int> &left, int left_stride,
                             const std::vector<int> &right, int right_stride) {
  auto safe =
  ...
  int correct = ip_with_stride(left.begin(), left.end(), left_stride,
                               right.begin(), right_stride, 0);
  if (right_stride != 1 && left_stride != 1) {
      std::cout << " test_sip : one stride should be 1\n";
      std::cout << "but the strides are "  << left_stride  <<  "  " << 
      right_stride << std::endl;
      return {0,1};
  }
  int result =
      (left_stride == 1 ? ip_with_strideR(left.begin(), left.end(),
                                          right.begin(), right_stride, 0)
                        : ip_with_strideL(left.begin(), left.end(), left_stride,
                                          right.begin(), 0));
