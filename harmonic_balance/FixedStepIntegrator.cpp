...
FixedStepIntegrator::FixedStepIntegrator(TemporalParameters t)
    : mTime(t.mStartTime),
      ...
      mNsteps(t.mNumStep[0]), mNumSkip(t.mNumSkip[0])
{}
...
void FixedStepIntegrator::IncrementTimeBeforeCallingTakeNextStep()
{
  mTime += StepSize();
}
...
void FixedStepIntegrator::PrepareLastStep(double timeAtEndOfStep, double initialTime)
{
...
void FixedStepIntegrator::PrintMe()
{
  std::cout << "FixedStepIntegrator: t = " 
  << mTime << " step " << mStepsTaken << " of " <<  mNsteps << "\n";
  std::cout << "      step sizes " << mCurrentStepSize << ", " << mInitialStepSize << 
  " total " << mTotalStepsTaken << " skip " << mNumSkip << "\n";
}