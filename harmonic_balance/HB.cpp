...
std::vector<double> interpolateHermite(double vm, double vp, double dvm, double dvp, double h) {
  ...
  double x2 = 1.0 / y2;
  double x3 = 1.0 / y3;
  std::vector<double> mat_col = {1.0, 0,  -3 * x2, 2 * x3, -1.0, 0, 3 * x2, 2 * x3,
                                 1.0, -x, -x2,     x3,     1.0,  x, -x2,    x3};
  ...
  if (std::abs(t1) < 0.5 * h) {
    ...
    for (int iter = 0; iter < 2; iter++) {
      ...
      if (t1 < -0.5 * h || t1 > 0.5 * h) {
        std::cout << "iteration " << iter << "  interval " << -0.5 * h << "," << 0.5 * h
                  << " init Newton step " << t1 << "\n";
        verbose = true;
      }
      ...
      if (verbose) {
        std::cout << "iteration" << iter << "  t " << t1 << "  u " << u1 << "  v " << vmin << "  a "
                  << amin << "\n";
      }
    }
  } else {
    // only needed once, when there was a negative local maximum of u(t),  u(t1
    std::cout << " init_disp: first Newton step " << t1 << " not in " << -0.5 * h << "," << 0.5 * h
              << "\n";
    verbose = true;
